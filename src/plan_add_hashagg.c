...
{
	char	   *name = get_opname(opexpr->opno);
	Expr	   *left;
	Expr	   *right;
	Expr	   *nonconst;

	if (list_length(opexpr->args) != 2 || strlen(name) != 1)
		return INVALID_ESTIMATE;
...
custom_group_estimate_integer_division(PlannerInfo *root, Oid opno, Node *left, Node *right)
{
	...
	if (strlen(name) == 1 && name[0] == '/' && IsA(right, Const))
	{
		Const	   *c = (Const *) right;
		int64		period;
...
custom_group_estimate_expr(PlannerInfo *root, Node *expr, double path_rows)
{
	if (IsA(expr, FuncExpr))
	{
		...
		if (strncmp(func_name, "time_bucket", NAMEDATALEN) == 0)
			return custom_group_estimate_time_bucket(root, func, path_rows);
		if (strncmp(func_name, "date_trunc", NAMEDATALEN) == 0)
			return custom_group_estimate_date_trunc(root, func, path_rows);
	}
